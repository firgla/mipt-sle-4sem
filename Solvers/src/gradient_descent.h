...
template<typename T>
std::vector<double> gradient_descent(CSR_Matrix<T>& matrix, const std::vector<T>& b,  const std::vector<T>& x_0, long double wanted_residual)
{
    ...
    while(residual > wanted_residual)
    {
        ...
        double tau = (scalar(err, err)) / (scalar(err, matrix * err));
        x = x_0_new - err * tau;
